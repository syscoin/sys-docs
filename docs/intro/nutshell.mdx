...
Simply put, as a community and an open source project, we develop and deploy the technology required to scale Bitcoin's ideals and proof-of-work security to the needs and evolving use-cases of a global population.

The project's vision, development over the years, and its design, means Syscoin has a significant head-start in evolving towards an all-encompassing protocol and network infrastructure that projects need in order to achieve Bitcoin L2.  Join us!

## The Blockchain Protocol

Syscoin is a mainnet Bitcoin L2 that provides [a data availability protocol that scales](../tech/poda), making EVM and AltVM rollups on Bitcoin a reality. [Merge-mined](../tech/merged-mining) by a majority Bitcoin's hashrate, Syscoin anchors rollups to Bitcoin's own proof-of-work and works with rollups’ existing sequencer architectures. 
...
Data Availability and scaling DA requires preventing the risks of impact from chain re-orgs. Syscoin solves this with a [finality mechanism of multi-quorum chainlocks](../tech/finality). This creates an additive security layer on top of Bitcoin's PoW. Served by 1,600 randomly selected [Sentry nodes](../tech/sentrynodes), this practically eliminates the risks of re-orgs, 51% attacks and selfish mining, without involving slashing.
