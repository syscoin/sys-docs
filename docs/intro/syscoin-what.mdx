...
Syscoin is a modular Proof-of-Work blockchain solution [merge-mined](/docs/tech/merged-mining) with Bitcoin. Syscoin is designed primarily to provide secure settlement and data availability, especially for rollups and other EVM or AltVM scaling layers that wish to tap into Bitcoin's anti-fragility and Proof-of-Work. Syscoin's base is composed of a dual-chain Layer 1: the core is the Syscoin native (UTXO) blockchain providing [data availability](/docs/tech/poda) and [finality](/docs/tech/finality). Running in tandem with the UTXO chain is an Ethereum Virtual Machine (EVM) chain called [*NEVM*](/docs/tech/nevm) (Network-Enhanced Virtual Machine), which is merged mined alongside the UTXO chain and also inherits finality.

Originally brought to mainnet in 2014, the Syscoin protocol has evolved through many iterations (much like blockchain technology itself), creating, optimizing and deprecating features or characteristics across time. 
...
- [**PoDA**](/docs/tech/poda) - Proof of Data Availability is a Bitcoin-based L1 DA protocol for rollups. Efficient, hash-based, general purpose, and easy to integrate.
- [**Rollux**](/docs/tech/rollux) - An OPStack rollup anchored to Bitcoin through Syscoin's DA protocol. Provides 2-second blocktime, extremely low fees, and supports application layers on top.  
- [**NEVM**](/docs/tech/nevm) - Network-Enhanced Virtual Machine is the L11 EVM of Syscoin, made specifically for supporting rollups and other scaling layers.
