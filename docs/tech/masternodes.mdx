...
Hosting a masternode requires 100k syscoin as collateral and while the masternode is active it will be paid regularly for its service. When exactly the masternode is paid is decided by when it goes online, it is added to the end of the queue of masternodes waiting to be paid each block and then will maintain that position in the queue as long as it is live.

The base amount of sys paid to a masternode is 58.62 sys. There are two further seniority levels for when the 100k syscoin transaction was sent as collateral as can be seen below.
...
If you already have 100k syscoin and are interested in setting up a masternode check out [this guide](../guides/mn_setup).
