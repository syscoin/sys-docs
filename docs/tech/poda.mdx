...
This means PoDA's purpose is to 1.) prove that raw data was properly made available to the public so that the public could archive it, and 2.) provide the on-chain basics (hash) needed to verify the integrity of any archived data at a later time.
