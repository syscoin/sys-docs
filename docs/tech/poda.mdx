...
PoDA's design is based on a separation of concerns between data availability and data archiving, with the protocol's purpose being the former.

This means PoDA purpose is to 

- Deterministically ensure and prove that raw data has been made available to the public and that the public was given a sufficient and defined opportunity to archive it.
- Provide the on-chain basics (hash) needed to verify the integrity of any archived data at a later time.

With PoDA, a hash of the raw data blob is stored on Layer 1, while an assumption is made that at least one honest party in the world will archive the raw data within at least a six-hour window of time - similar to the honesty assumption made when syncing a Bitcoin node (at least one honest node).
...
Archiving is a seperate concern from data availability. With PoDA, the focus is DA, and archiving is left non-determined. However, tools are available.

Archivers can utilize Syscoin's Sentinel implementation below which provides the means to both store and retrieve PoDA data blobs, or employ other means at their discretion. If decentralized archiving is a priority, there are networks that you can custom-integrate with PoDA, such a KYVE.
