...
Syscoin is designed with this in mind. All near-instant activity on Rollux inherits the full security of Syscoin’s L1 in the background, including finality.
...
### Finality that is Fast, Decentralized and Fault Tolerant
...
While the Syscoin network currently provides finality on every block, this goes beyond what is projected to be necessary. Every five blocks is more efficient (every 12.5 minutes based on average blocktime). This would provide sufficient security with lower possibility of faults. This timing will be implemented in the next Syscoin Core upgrade (v4.4). In the future, if it is determined that more frequent finality has become more ideal, the network can shift to that mode of operation.   
...
**Q."Does Syscoin Rollux have a gas token?"** 

A. No. It uses the native coin of Syscoin (SYS, or TSYS in the case of testnet) for gas.


**Q."How do I move some TSYS to the Rollux testnet?"** 

A. A bridge contract is available. You simply send TSYS to a contract address on the Tanenbaum L1, then it arrives at your address on Rollux. **[Refer to instructions](/docs/guides/rollux/metamask#how-to-get-tsys-andor-bridge-tsys-to-rollux)**.
...
A. Every 35-40 minutes on average. The L2 will only make an L1 commitment if there is at least one new transaction on the L2 which has not yet been settled on L1.